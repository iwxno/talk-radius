...
const Editpopup = ({ setEditPopup }) => {
  ...
  const { updateProfile, isEditingRoom } = useRoomStore();
  const { selectedRoom } = useMessageStore();
  const [username, setUsername] = useState(selectedRoom.name);
  const [imagePreview, setImagePreview] = useState(selectedRoom.roomImg);
  const [imageBase64, setImageBase64] = useState("");

  const handleCameraClick = async () => {
    await updateProfile(username, imageBase64, selectedRoom._id);
    setEditPopup(false);
  };
      ...
      <div className="relative w-full max-w-sm bg-[#1a1a1a]/80 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 shadow-2xl overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="flex items-center justify-between gap-2 w-full mb-8">
            ...
            <X onClick={() => setEditPopup(false)} className="cursor-pointer" />
          </div>
